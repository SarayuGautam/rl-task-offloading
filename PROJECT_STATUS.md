...
and the saved artifacts on 2026-09-01, not copied from the report.
...
| Statistical validation (5 seeds, 95% CI) | ✅ Done — **+9.13% ± 1.05%** vs Always Cloud |
...
| Pareto latency-energy trade-off sweep | ⚠️ Implemented; **on-disk artifact is stale** (see Discrepancies) |
| DQN implementation | ❌ Stub only (thesis-phase extension) |
| Final report | 🔄 In finalisation |
...
## ⚠️ Discrepancies between the report text and the saved artifacts

Found 2026-09-01 by recomputing from `qlearning_trained.npz` and
`pareto_data.csv`. These are numbers an examiner can recompute, so fix the text.

1. **Count of barely-visited states.** §4.4 says "three visited states with a
...
2. **Visit range for the well-trained region.** §4.4 says the states with queue
   length below three "was visited between roughly 3,000 and 144,000 times per
   action." Across both bins that qualify (0 and 1) the true span is
   **210 – 144,111**. The ~3,000 floor holds only for queue bin 0 alone, where
   the true minimum is 6,401.
3. **Q(local) range.** §4.4 says Q(local) "lies between −1.36 and −1.63." The
   true range over the well-trained bins is **−1.361 to −1.742**.
4. **Pareto sweep is stale on disk.** `pareto.py` defines
   `BETAS = [0.0, 0.15, 0.3, 0.5, 0.7, 0.85, 1.0]` (seven weightings), but the
   cached `experiments/results/pareto_data.csv` holds only **four** rows
   (w_latency 1.0 / 0.7 / 0.3 / 0.0) with energy 1.9302e-4 and 1.93048e-4 —
   i.e. 1.93 × 10⁻⁴, not the 1.94 × 10⁻⁴ quoted in the report and the defense
   briefing. Both the CSV and `charts/pareto_curve.png` are dated 18 Aug 17:32
   and come from an earlier four-point version of the sweep. **Re-run
   `--agent pareto` to regenerate both**, then re-check the quoted constant and
   the "seven settings" wording. The cache check in `run_pareto()` will miss and
   force a full re-sweep, so this costs 7 × 1,500 training episodes.

The qualitative claims in §4.4 and §4.5 are unaffected — local really is never
selected, and energy really is invariant over the realised action set. Only the
specific figures above need correcting.
...
1. **Re-run `--agent pareto`** to regenerate `pareto_data.csv` and
   `pareto_curve.png` at the seven weightings the code now specifies, then
   reconcile the energy constant quoted in §4.5 (1.93 vs 1.94 × 10⁻⁴).
2. **Correct the four §4.4 figures** listed under Discrepancies.
3. **Commit the `main.py` eval-seed change.**
